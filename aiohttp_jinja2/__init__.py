...
_ContextProcessor = Callable[[web.Request[_T]], Awaitable[Dict[str, Any]]]
...
def setup(
    ...
    *args: Any,
    app_key: str = APP_KEY,
    context_processors: Iterable[_ContextProcessor[Any]] = (),
    ...
    **kwargs: Any,
    ...
    app.state[app_key] = env
    if context_processors:
        app.state[APP_CONTEXT_PROCESSORS_KEY] = context_processors
...
def get_env(app: web.Application[Any], *, app_key: str = APP_KEY) -> jinja2.Environment:
    return cast(jinja2.Environment, app.state.get(app_key))
...
async def context_processors_middleware(
    request: web.Request[_T],
    handler: Callable[[web.Request[_T]], Awaitable[web.StreamResponse]],
) -> web.StreamResponse:
